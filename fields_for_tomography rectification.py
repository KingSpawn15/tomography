...
def polarized_source(sigma_t_sec, t0_sec, source_x, source_y):
    return [mp.Source(src=mp.CustomSource(src_func=current_rectification(sigma_t_sec, t0_sec, 1),
                                         is_integrated=True),
...
if __name__ == '__main__':
    ...
    sigma_t_sec = fwhm_t_fs * 1e-15 / np.sqrt(8 * np.log(2))
    source_x = 0
    source_y = - (1 / inas["alpha"])
    source_rectification = polarized_source(sigma_t_sec, t0_sec, source_x, source_y)
    ...
    if mp.am_master():
        ...
        plt.colorbar()
        plt.savefig(path + '/orfield_'+ str(intensity) + 'fwhm_t_' + str(fwhm_t_fs)+'.png', dpi=300)
        ...
        np.save(path +"er_fields_3d_rectification.npy", ex_fields_3d)
        np.save(path +"ex_fields_3d_rectification.npy", ey_fields_3d)
